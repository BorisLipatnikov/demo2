...
// выбор функции обработки параметра
    char type_respons = E120_RESPONSE_TYPE_ACK ;
    char Get = 0; // if Get == 0 -> Set
    if (rdm.Class == E120_GET_COMMAND) Get = 1; // Get
          
    switch (rdm.PID) {
        case (E120_DISC_UNIQUE_BRANCH):
            if (rdm.mute) return;            
            if (Discovery_Unique_Branch_Message (&rdm.buffer[0])) 
                RDM_Transmit (0); 
            return;
        case(E120_DISC_MUTE):
            rdm.mute = 1;
        ...
        case (E120_DEVICE_INFO):
            Device_Info (rdm.PDL, dmx.start_address, Software_Version_ID[0], \
                    Personality, Total_Personality);
            break;        
        ...
        case (E120_DEVICE_LABEL):
            if (Get) Device_Label_Get (rdm.PDL);
            else {
        ...
        case(E120_SOFTWARE_VERSION_LABEL):
            Software_Version_Label (rdm.PDL, Software_Version_ID);
            break;
        case(E120_IDENTIFY_DEVICE):
            if (Get) Identify_Device_Get (rdm.PD, &rdm.Identif);
            else Identify_Device_Set (rdm.PD, &rdm.Identif);            
            break;
        case (E120_DMX_START_ADDRESS):
            if (Get) DMX_Starting_Address_Get (rdm.PD, dmx.start_address);
            else {
                DMX_Starting_Address_Set (rdm.PD, &dmx.start_address);
                rdm.write = 1;
            }
            break;
        case(E120_DMX_PERSONALITY):
            if (Get) DMX_Personality_Get (rdm.PD, Personality, Total_Personality);
            else {
                Personality_Set (DMX_Personality_Set (rdm.PDL));                
                rdm.write = 1;
            }
            break;
        ...
        case (E120_SUPPORTED_PARAMETERS):
            type_respons = RDM_Support_parametr (rdm.PDL);
            break;
        ...
        case (E137_1_CURVE):
            if (Get) {
                *rdm.PDL = 2; 
                *rdm.PD = current_curve+1;
                *(rdm.PD+1) = TOTAL_CURVE; 
            } 
            else {
                current_curve = (*rdm.PD)-1;
                *rdm.PDL = 0;
                rdm.write = 1;
            }
            break;
        ...
        case (E137_1_MODULATION_FREQUENCY):
            if (Get) {
                *(rdm.PDL) = 2; 
                *(rdm.PD) = pwm.Current_Modulation_Freq;
                *(rdm.PD+1) = TOTAL_MODULATION_FREQUENCY; 
            }
            else {
                pwm.Current_Modulation_Freq = *rdm.PD;
                Set_PWM_Freq (pwm.Current_Modulation_Freq);
                *rdm.PDL = 0;
                rdm.write = 1;
            }
            break;
        case (E137_1_MODULATION_FREQUENCY_DESCRIPTION):
            if (*rdm.PD > TOTAL_MODULATION_FREQUENCY) 
                ...
                *rdm.PDL = Modulation_Frequency_Description (rdm.PD+1, Calc_Freq(*rdm.PD));
            break;
          */
        default:            
