...
void switch_device_mode (mode_t new_mode){
    di();
    // список контролируемых обнул€емых переменных
...
void HandlerMenu(void) {
    ...
    // вкл диспле€ при первом нажатии с блокировкой функции кнопки    
    ...
    switch(menu.focus) {
        // первоначальное отображение и переадресаци€ при вкл 
        case(item_start):  
        ...
        case(item_manual_mode):
            ...
            // восстанавливает предыдущий режим 
                ...
                motor.target_speed = 60; // ед дмикс, подбираетс€ на глаз
            }            
            break;
...
void RunMotor (uint16_t value) {    
    ...
    // откл питани€ до перезагрузки прибора
...
void TimerControlBlock (void) {
    ...
    // фиксаци€ перехода 0->1 , прерыватель на моторе, признак вращени€
